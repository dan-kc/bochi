...
struct FrequencyConversionTests {
    ...
    // Behaviour: When a user sets a recurringTask to "3 times per day", the internal daily
    ...
    // Behaviour: When a user sets a recurringTask to "3 times per week", the daily rate
    ...
    // Behaviour: When a user sets a recurringTask to "3 times per month", the daily rate
    ...
    // Behaviour: When a recurringTask has a daily frequency, the user sees a compact
    // summary like "3/day" in the recurringTask list.
    @Test func formatSummaryDay() {
        let summary = FrequencyConversion.formatSummary(3.0)
        #expect(summary == "3/day")
    }

    // Behaviour: When a recurringTask has a weekly frequency, the summary shows "3/week".
    @Test func formatSummaryWeek() {
        let summary = FrequencyConversion.formatSummary(3.0 / 7.0)
        #expect(summary == "3/week")
    }

    // Behaviour: When a recurringTask has a monthly frequency, the summary shows "1/month".
    @Test func formatSummaryMonth() {
        let summary = FrequencyConversion.formatSummary(1.0 / 30.0)
        #expect(summary == "1/month")
    }

    // Behaviour: When a recurringTask has no frequency set, no summary is displayed.
