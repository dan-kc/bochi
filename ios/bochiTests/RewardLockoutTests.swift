...
struct RewardLockoutTests {
    ...
    // Behaviour: a lapsed premium user can buy through a saved reward lockout,
    // but re-enabling premium should restore the cooldown from that purchase.
    @Test func lapsedPremiumIgnoresLockoutButTracksPurchasesForReenabledPremium() {
        let tradeStore = makeStore()
        let reward = makeReward(lockoutDurationSeconds: 3 * 3_600)
        let purchaseDate = Date(timeIntervalSince1970: 1_800_000_000)
