...
struct Habit: Identifiable, Equatable, Sendable {
    ...
    let difficultyRank: String? // Fractional index key for difficulty ordering.
                                // nil = not ranked yet. Higher keys = harder habits.
                                // Generated by DifficultyRanker's binary search.

    // Whether this habit has the required properties to calculate a reward price.
    // Both frequency and difficulty rank must be set for the reward formula to
    // produce meaningful results — without frequency, F is always 1 (no diminishing
    // returns), and without difficulty rank, D defaults to 0.5 (no relative positioning).
    var canTrade: Bool {
