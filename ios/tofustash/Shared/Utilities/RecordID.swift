...
// creating a habit/reward locally and then receiving the same row back from
// sync must still render as one item, not a duplicate.
struct RecordID: RawRepresentable, Hashable, Codable, Sendable, ExpressibleByStringLiteral, CustomStringConvertible {
    let rawValue: String
