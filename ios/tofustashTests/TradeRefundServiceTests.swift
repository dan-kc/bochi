...
struct TradeRefundServiceTests {
    ...
    @Test("task trade refunds reopen the task and can be undone")
    func taskTradeRefundReopensTaskAndCanBeUndone() throws {
        ...
        tradeStore.addTaskTrade(id: "trade-1", taskId: task.id, amount: 120, createdAt: completedAt, shouldNotifySync: false)
        ...
        let reopenedTask = try #require(taskStore.tasks.first(where: { $0.id == task.id }))
        let refundedTrade = try #require(tradeStore.trades.first(where: { $0.id == initialTrade.id }))
        #expect(reopenedTask.completedAt == nil)
