...
// Sync flow: durable ledger for cursor state, full-sync requirements, and dirty
// record generations written by stores before lifecycle hooks trigger network sync.
@MainActor
final class SyncStateStore {
    ...
    struct DirtyState: Equatable {
        var timers: [DirtyRecordVersion] = []
        var tasks: [DirtyRecordVersion] = []
        var recurringTasks: [DirtyRecordVersion] = []
        ...
        var taskRecurringTaskDependencies: [DirtyRecordVersion] = []
        var recurringTaskTags: [DirtyRecordVersion] = []
        var rewards: [DirtyRecordVersion] = []
        var rewardTaskDependencies: [DirtyRecordVersion] = []
        var rewardRecurringTaskDependencies: [DirtyRecordVersion] = []
        var rewardTags: [DirtyRecordVersion] = []
        var themePalettesGeneration: Int64?

        var themePalettes: Bool {
            themePalettesGeneration != nil
        }
    }
    ...
    func state(for userID: String, on databaseHandle: AppDatabaseHandle) throws -> UserSyncState {
        ...
        return UserSyncState(
            ...
            dirty: DirtyState(
                timers: dirtyRecords[.timers] ?? [],
                tasks: dirtyRecords[.tasks] ?? [],
                recurringTasks: dirtyRecords[.recurringTasks] ?? [],
                ...
                taskRecurringTaskDependencies: dirtyRecords[.taskRecurringTaskDependencies] ?? [],
                recurringTaskTags: dirtyRecords[.recurringTaskTags] ?? [],
                rewards: dirtyRecords[.rewards] ?? [],
                rewardTaskDependencies: dirtyRecords[.rewardTaskDependencies] ?? [],
                rewardRecurringTaskDependencies: dirtyRecords[.rewardRecurringTaskDependencies] ?? [],
                rewardTags: dirtyRecords[.rewardTags] ?? [],
                themePalettesGeneration: dirtyFlags[.themePalettes]
            )
        )
    }
    ...
    private func clearSnapshot(
        ...
        let dirtyRecordGroups: [(kind: SyncEntityKind, versions: [DirtyRecordVersion])] = [
            (.timers, snapshot.dirty.timers),
            (.tasks, snapshot.dirty.tasks),
            (.recurringTasks, snapshot.dirty.recurringTasks),
            (.trades, snapshot.dirty.trades),
            (.tags, snapshot.dirty.tags),
            (.taskTags, snapshot.dirty.taskTags),
            (.taskTaskDependencies, snapshot.dirty.taskTaskDependencies),
            (.taskRecurringTaskDependencies, snapshot.dirty.taskRecurringTaskDependencies),
            (.recurringTaskTags, snapshot.dirty.recurringTaskTags),
            (.rewards, snapshot.dirty.rewards),
            (.rewardTaskDependencies, snapshot.dirty.rewardTaskDependencies),
            (.rewardRecurringTaskDependencies, snapshot.dirty.rewardRecurringTaskDependencies),
            (.rewardTags, snapshot.dirty.rewardTags)
        ]

        for dirtyRecordGroup in dirtyRecordGroups {
            try clearDirty(
                userID: userID,
                kind: dirtyRecordGroup.kind,
                versions: dirtyRecordGroup.versions,
                on: databaseHandle
            )
        }

        if let generation = snapshot.dirty.themePalettesGeneration {
            try database.execute(
                """
                DELETE FROM dirty_flags
                WHERE user_id = ? AND entity_kind = ? AND mutation_generation = ?
                """,
                bindings: [.text(userID), .text(SyncEntityKind.themePalettes.rawValue), .int(generation)],
                on: databaseHandle
            )
        }
    }
