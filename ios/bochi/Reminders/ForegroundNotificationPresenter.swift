...
final class ForegroundNotificationPresenter: NSObject, UNUserNotificationCenterDelegate {
    ...
    nonisolated func userNotificationCenter(
        ...
        let recurringTaskID = response.notification.request.content.userInfo["recurringTaskID"] as? String
        DispatchQueue.main.async {
            if let taskID {
                self.handleNotificationRoute(.task(RecordID(taskID)))
            } else if let recurringTaskID {
                self.handleNotificationRoute(.recurringTask(RecordID(recurringTaskID)))
            }
            completionHandler()
        }
    }
