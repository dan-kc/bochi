...
fn register_and_login_user(server: &SharedTestServer, email: &str, password: &str) -> Result<(String, String), String> {
    ...
    let body = tokens_response.into_string().map_err(|e| format!("Failed to read response: {}", e))?;
    let json: serde_json::Value = serde_json::from_str(&body).map_err(|e| format!("Failed to parse JSON: {}", e))?;
...
fn test_refresh_tokens_malformed_token() {
    ...
    assert!(response.is_err(), "Refresh with malformed token should fail");
    if let Err(ureq::Error::Status(code, response)) = response {
...
fn test_refresh_tokens_multiple_times() {
    ...
    for i in 0..3 {
        ...
        let new_refresh_token = json.get("refreshToken")
            .and_then(|v| v.as_str())
            .expect("Should have new refreshToken");
        let _new_access_token = json.get("accessToken")
            .and_then(|v| v.as_str())
