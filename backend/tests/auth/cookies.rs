...
#[tokio::test]
async fn test_graphql_auth_with_cookie() {
    let email = generate_email_from_fn!(test_graphql_auth_with_cookie);
    let password = "password123";
    ...
    // Get access token from response body (we'll use it to construct cookie)
    let login_body_bytes = login_response
        .into_body()
        .collect()
        .await
    ...
    // Make GraphQL request with cookie instead of Authorization header
    let router2 = router::router().await;
    let query = json!({
        "query": "{ tasks { id } }"
    });
    ...
    let response = router2
        .oneshot(
            Request::builder()
                ...
                .uri("/graphql")
                .header(http::header::CONTENT_TYPE, "application/json")
                .header(http::header::COOKIE, format!("access_token={}", access_token))
    ...
    assert_eq!(
        response.status(),
        StatusCode::OK,
        "GraphQL request with cookie auth should succeed"
    );

    let body_bytes = response.into_body().collect().await.unwrap().to_bytes();
    let json: serde_json::Value = serde_json::from_slice(&body_bytes).unwrap();

    // Should have data, not errors (or at least not auth errors)
    assert!(
        json.get("data").is_some(),
        "Response should have data field. Got: {:?}",
        json
    );
}

#[tokio::test]
async fn test_graphql_prefers_authorization_header_over_cookie() {
    let email1 = generate_email_from_fn!(test_graphql_prefers_authorization_header_over_cookie_1);
    let email2 = generate_email_from_fn!(test_graphql_prefers_authorization_header_over_cookie_2);
    let password = "password123";
    ...
    // Login as user1 to get their token
    let router = router::router().await;
    let login_body = json!({
        "email": email1,
        "password": password
    ...
    let body_bytes = login_response
        ...
        .to_bytes();
    let json: serde_json::Value = serde_json::from_slice(&body_bytes).unwrap();
    let user1_token = json["accessToken"].as_str().unwrap();

    // Login as user2 to get their token
    let router2 = router::router().await;
    ...
    let body_bytes2 = login_response2
        .into_body()
        .collect()
        .await
        .unwrap()
    ...
    let user2_token = json2["accessToken"].as_str().unwrap();

    // Make request with user1 token in Authorization header and user2 token in cookie
    // Should use user1 (Authorization header takes precedence)
    let router3 = router::router().await;
    let query = json!({
        "query": "{ tasks { id } }"
    });
        ...
        .oneshot(
            Request::builder()
                ...
                .header(
                    ...
                    format!("Bearer {}", user1_token),
                )
                .header(http::header::COOKIE, format!("access_token={}", user2_token))
                .body(Body::from(query.to_string()))
                .unwrap(),
        )
        .await
    ...
    // Request should succeed (we're just verifying it doesn't error)
    assert_eq!(
        response.status(),
        StatusCode::OK,
        "Request with both auth methods should succeed using Authorization header"
    );
}
