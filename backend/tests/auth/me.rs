...
#[tokio::test]
async fn test_trailing_slash_get_me_uses_same_route() {
    // The app and manual API checks should not fail just because the account
    // endpoint includes a trailing slash.
    let (status, json) = make_unauthenticated_get_request("/auth/me/").await;
...
async fn test_get_me_returns_registered_user_default_subscription_state() {
    ...
    assert_eq!(
        json.get("subscriptionProductId"),
        Some(&serde_json::Value::Null),
        "Without a linked purchase there should be no Apple product id"
    );
    assert_eq!(
