# ECS Task Definition
resource "aws_ecs_task_definition" "bochi_server" {
  family                   = "bochi-server"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        {
          name  = "APPLE_SIGN_IN_AUDIENCE"
          value = "app.bochi"
        },
        {
          name  = "APPLE_SIGN_IN_TEAM_ID"
          value = var.apple_sign_in_team_id
        },
        {
          name  = "APPLE_SIGN_IN_KEY_ID"
          value = var.apple_sign_in_key_id
        },
        {
          name  = "APP_STORE_SERVER_ISSUER_ID"
          value = var.app_store_server_issuer_id
        },
        {
          name  = "APP_STORE_SERVER_KEY_ID"
          value = var.app_store_server_key_id
        },
        {
          name  = "APP_STORE_SERVER_BUNDLE_ID"
          value = var.app_store_server_bundle_id
        },
        {
          name  = "DB_HOST"
      ...
      secrets = [
        ...
        {
          name      = "JWT_PUBLIC_KEY"
          valueFrom = "${data.aws_secretsmanager_secret.jwt_keys.arn}:eddsa-public-key::"
        },
        {
          name      = "JWT_PRIVATE_KEY"
          valueFrom = "${data.aws_secretsmanager_secret.jwt_keys.arn}:eddsa-private-key::"
        },
        {
          name      = "APP_STORE_SERVER_PRIVATE_KEY"
          valueFrom = data.aws_secretsmanager_secret.app_store_in_app_purchase_key.arn
        },
        {
          name      = "APPLE_SIGN_IN_PRIVATE_KEY"
          valueFrom = data.aws_secretsmanager_secret.apple_sign_in_private_key.arn
        },
      ]
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"         = aws_cloudwatch_log_group.bochi_server.name
  ...
  tags = {
    Name = "bochi-server-task-definition"
  }
}

# Security group for ECS tasks
resource "aws_security_group" "ecs_tasks" {
  name        = "bochi-server-ecs-tasks-sg"
...
resource "aws_cloudwatch_log_group" "bochi_server" {
  name              = "/ecs/bochi-server"
  retention_in_days = 7 # Keep logs for 7 days to save costs

  tags = {
    Name = "bochi-server-logs"
  }
}

# Security group for EC2 instances
resource "aws_security_group" "ecs_instances" {
  name        = "bochi-ecs-instances-sg"
...
resource "aws_launch_template" "ecs_instance" {
  name_prefix   = "bochi-ecs-instance-"
...
resource "aws_ecs_capacity_provider" "ec2" {
  name = "bochi-ec2-capacity-provider"
...
resource "aws_ecs_cluster_capacity_providers" "bochi" {
...
resource "aws_lb_target_group" "bochi_server" {
  name     = "bochi-server-tg-blue"
...
resource "aws_ecs_service" "bochi_server" {
  name            = "bochi-server"
  cluster         = var.ecs_cluster_id
  task_definition = aws_ecs_task_definition.bochi_server.arn
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.bochi_server.arn
    container_name   = "bochi-server"
    container_port   = 8080
  }
  ...
  depends_on = [
    aws_lb_listener_rule.bochi_server,
    aws_iam_role_policy.ecs_task_execution_secrets,
  ]

  tags = {
    Name = "bochi-server-service"
  }
}

# HTTPS Listener Rule
# Attaches to the HTTPS listener created in lb.tf.
resource "aws_lb_listener_rule" "bochi_server" {
