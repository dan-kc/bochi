...
fun RewardFormSheet(
    ...
    onSave: (name: String, description: String, maxDailyFrequency: Double?) -> Unit,
    onDelete: ((String) -> Unit)?,
    onRankDamage: ((String) -> Unit)?,
    onDismiss: () -> Unit,
) {
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            Text(
                text = if (formState.isEditing) "Edit Reward" else "New Reward",
            ...
            if (formState.isEditing) {
                ...
                if (onRankDamage != null) {
                    OutlinedButton(
                        onClick = { onRankDamage(formState.editingReward!!.id) },
                        shape = RoundedCornerShape(12.dp),
                        ...
                        Text(
                            if (formState.editingReward?.damageRank != null) "Re-rank Damage"
                            else "Set Damage",
                        )
                    }
                }
                ...
                if (onDelete != null) {
                    Spacer(Modifier.height(8.dp))
                    OutlinedButton(
                        onClick = { onDelete(formState.editingReward!!.id) },
                        shape = RoundedCornerShape(12.dp),
                        colors = ButtonDefaults.outlinedButtonColors(
