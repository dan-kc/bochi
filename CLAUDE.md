...
**CRITICAL:** NEVER run `./gradlew` directly in the `android/` directory. On NixOS, AGP's Maven-downloaded aapt2 binary is dynamically linked and will fail. All Gradle commands MUST go through the `agradle` wrapper (or the shorthand commands below), which passes `-Pandroid.aapt2FromMavenOverride` pointing to the Nix-patched aapt2.

- `ab` - Build debug APK
- `abr` - Build release APK
- `at` - Run JVM unit tests (all tests run via Robolectric, no emulator needed)
- `alint` - Lint Kotlin code (ktlint)
- `afmt` - Auto-format Kotlin code (ktlint)
- `ai` - Install + launch on connected device
